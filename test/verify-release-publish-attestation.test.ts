...
test("an unknown array reference is left untouched rather than erased", () => {
  // expandArrays replaces "${name[@]}" with the array's contents, but an unknown
  // name is left as-is so the scan does not silently drop a flag it does not
  // understand. Covers the ?? fallback in the inline version's expandArrays.
  const result = auditPublishAttestation([
    { file: "release.yml", text: `          npm publish "\${unknown[@]}"` },
  ]);
  assert.equal(result.failures.length, 1, "the untouched reference has no --provenance");
});

test("stripComment handles a backslash escape and a single quote outside double quotes", () => {
  // The inline verifier carries stripComment here; the import version delegates
  // it to verify-release-changelog-date.ts. A backslash escapes the next
  // character (covers the \\ branch) and a single quote outside double quotes
  // toggles single-quote mode (covers the single = !single branch).
  const result = publishInvocationsIn({
    file: "release.yml",
    text: `          npm publish --access public \\# --provenance --ignore-scripts '`,
  });
  assert.equal(result.length, 1);
  assert.equal(attestationEnabled(result[0]!.command), true, "the escaped hash and trailing quote do not remove --provenance");
});
