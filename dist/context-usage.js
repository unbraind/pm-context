...
export function resolveSince(value, now = new Date()) {
    ...
    if (relative)
        return new Date(now.getTime() - Number(relative[1]) * 86_400_000).toISOString();
...
export function parseLedgerLine(line) {
    ...
    if (row.kind !== "serve")
        return null;
    if (typeof row.profile !== "string" || !Array.isArray(row.rows))
        return null;
    ...
    for (const candidate of row.rows) {
        ...
        if (typeof entry.id !== "string" || typeof entry.rank !== "number" || typeof entry.included !== "boolean")
            return null;
 ...
 */
export function readLedger(pmRoot) {
...
export function buildUsageReport(events, options = {}) {
    ...
    const selected = events.filter((event) => {
        ...
        if (since && event.at <= since)
            return false;
        if (surface && event.kind === "serve" && event.surface !== surface)
            return false;
        return true;
    });
    const items = new Map();
    const touchTimesByAuthor = new Map();
    const judgments = [];
    ...
    for (const event of selected) {
        ...
        if (from === null || event.at < from)
            from = event.at;
        if (to === null || event.at > to)
            to = event.at;
        if (event.kind === "touch") {
            touchEvents += 1;
            const entry = entryFor(event.item_id);
            entry.touches += 1;
            if (entry.last_touched_at === null || event.at > entry.last_touched_at)
            ...
            if (times)
                ...
                touchTimesByAuthor.set(key, [event.at]);
            continue;
        }
        serveEvents += 1;
    ...
    const ranked = [...items.values()].sort((a, b) => b.serves - a.serves || b.conversions - a.conversions || a.id.localeCompare(b.id));
 ...
 */
export function reportContextUsage(pmRoot, options = {}) {
...
export function renderUsageReport(report) {
    ...
    if (!report.ledger_present) {
        lines.push("No ledger at `runtime/context-usage.jsonl`.", "", "pm writes it on the first `pm context`, `pm next`, or item mutation.", "");
        return lines.join("\n");
    }
    const rate = report.conversion_rate === null ? "n/a" : `${(report.conversion_rate * 100).toFixed(1)}%`;
    lines.push(`- serve-then-touch conversion: **${rate}**`, `- events: ${report.event_count} (${report.serve_event_count} serve, ${report.touch_event_count} touch)`, `- window: ${report.from ?? "n/a"} .. ${report.to ?? "n/a"}`, `- authors: ${report.authors.length > 0 ? report.authors.join(", ") : "n/a"}`, `- surfaces: ${report.surfaces.length > 0 ? report.surfaces.join(", ") : "n/a"}`);
    if (report.malformed_line_count > 0)
        lines.push(`- malformed lines skipped: ${report.malformed_line_count}`);
    lines.push("");
