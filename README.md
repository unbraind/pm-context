...
per-clone Git config; `npm install` / `npm ci` wires them automatically via the `prepare` script (a portable Node guard, `scripts/prepare-merge-driver.ts`, the canonical pm-ops launcher template, copied unchanged and checked byte-for-byte by a test: it runs
